		...
		public function updateModifierAction() {
			...
			if ( !empty( $_POST ) ) {
				...
				try {
					...
					$newCommentaire = htmlentities(
						$inputValidator->validateString( $_POST[ 'commentaire' ] )
					);

					//On regarde si l'utilisateur veur ajouter une catégorie
					if ( empty( $_POST[ 'nouvelleCategorie' ] ) ) {
						$newCategory = htmlentities(
							$inputValidator->validateString( $_POST[ 'selectList' ] )
						);
					} else {
						$newCategory = htmlentities(
							$inputValidator->validateString( $_POST[ 'nouvelleCategorie' ] )
						);
					}
